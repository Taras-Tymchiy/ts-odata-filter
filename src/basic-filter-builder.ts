...
import {
  IBasicFilterBuilder,
  ICollectionPropFilterBuilder,
  IFilterBuilderResult,
  IFilterExpressionResult,
  BasicBuilderFunc,
  BasicBuilderArg,
  BasicBuilderArgFunc,
  BasicBuilderProp,
...
export class FilterExpressionResult<TEntity, TResult>
  implements IFilterExpressionResult<TEntity, TResult> {
...
export class BasicFilterBuilder<T> implements IBasicFilterBuilder<T> {
  ...
  getArgString<
    TResult extends TPropType,
    TBuilder extends BasicFilterBuilder<T>
  >(arg: BasicBuilderArg<T, TBuilder, TResult>): string {
  ...
  function<TResult extends TPropType, TBuilder extends BasicFilterBuilder<T>>(
    ...
    return new FilterExpressionResult<T, TResult>(
      `${functionName}(${argsString})`
    )
  }

  binaryOperator<
    TArg extends TPropType,
    TBuilder extends BasicFilterBuilder<T>
  >(
    op: string,
  ...
  protected logicalOperator<
    TArg extends TPropType,
    TBuilder extends BasicFilterBuilder<T>
  >(op: string, ...args: BasicBuilderArg<T, TBuilder, TArg>[]) {
    if (!args.length) {
  ...
  protected collectionProp<I, TBuilder extends BasicFilterBuilder<I>>(
    prop: ObjPathProxy<T, Array<I>>,
    varName: string | null
  ): ICollectionPropFilterBuilder<I, TBuilder> {
...
class CollectionPropFilterBuilder<I, TBuilder extends BasicFilterBuilder<I>>
  ...
  constructor(
    private collectionProp: ObjPathProxy<any, Array<I>>,
  ...
  private collectionFunc(
    name: string,
    condition: BasicBuilderArgFunc<I, TBuilder, boolean> | null
  ) {
    ...
    return new FilterExpressionResult<I, boolean>(
