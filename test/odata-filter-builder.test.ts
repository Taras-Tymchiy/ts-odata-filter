    ...
    const filters: FilterTest<IProduct>[] = [
      {
        ...
        filter: ({eq, replace}, p) =>
          eq(replace(p.name, 'test', 'x'), 'xx'),
        expectedResult: `(replace(name, 'test', 'x') eq 'xx')`
      }, {
        filter: ({eq, substring}, p) =>
          eq(substring(p.name, 1), 'Phone'),
    ...
    const filters: FilterTest<IOrder>[] = [
      {
        ...
        filter: ({eq, date}, p) => eq(date(p.createdDate), new Date('2000-01-01')),
        expectedResult: `(date(createdDate) eq 2000-01-01T00:00:00.000Z)`
      }, {
        filter: ({eq, time}, p) => eq(time(p.createdDate), new Date('2000-01-01')),
...
describe("ODataFilterBuilder collections test", () => {
  it("Renders collections filters", () => {
    ...
    const filters: FilterTest<IOrder>[] = [
      {
        filter: (b, p) => b.collection(p.orderItems, 'i').any(i=>
          i.lt(i.prop.price, 100)
        ),
        expectedResult: `orderItems/any(i: (i/price lt 100))`
      }, {
        filter: (b, p) => b.collection(p.orderItems, 'i').all(i=>
          i.lt(i.prop.price, 100)
        ),
...
describe("ODataFilterBuilder nested test", () => {
  it("Allows to reuse filters via nested()", () => {
    ...
    expect(ODataFilterBuilder.build(orderTest.filter).getString())
      .toEqual(orderTest.expectedResult)
    expect(ODataFilterBuilder.build(orderItemTest.filter).getString())
      .toEqual(orderItemTest.expectedResult)
